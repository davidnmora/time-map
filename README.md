...
-  [World Historical Gazetteer](https://whgazetteer.org/public_data/): "a prize-winning digital infrastructure platform for integrating databases of historical place name information"
- [The Megalithic Portal](https://www.megalithic.co.uk/search.php) is a web resource dedicated to prehistoric archaeology and closely related subjects

### technical tools / specifications
- MF-JSON (Moving Features JSON) is an Open Geospatial Consortium (OGC) standard for encoding the movement of geographic features, such as vehicles or people, in JSON
...
#### Time Map... exists already?

[RunningReality.org](https://www.runningreality.org/#01/01/1200&43.59449,24.97289&zoom=5) is easily the most sophisticated, comprehensive, actively evolving historical mapping project. Think of it less as a single map or dataset, and more as "history engine" (like a search engine or the engine of a train). It began 25 years ago [by this NASA dude](https://www.linkedin.com/in/garth-henning/) (hence the ambition/sophistication?), who started working on it full time in July 2025. Their aim is to basically map all of history in incredible detail via community contributions and vetting of sources, letting you pull in sources stored elsewhere.

They [describe](https://www.runningreality.org/docs/about/faq.jsp) themselves: "we have some countries on each continent in each year since 3000BC. We are also working on cities and armies and battles and ships. The goal is eventually to get down to street-level views of all history." 

They seem all about people using their maps/data, but also their data seems distributed: from their FAQ, "*Can I export all the nation borders?* No. Given the complexity of our history engine, we are not sure how exactly to do this. It is not a static map like a normal web map with all the data synchronized at the same time where you can, say, get all the polygons for 1700AD." That said, if we used their data, we wouldn't need data for one a specific year, we'd want the underlying data specifying the geography, when it was, and any metadata (place name, etc). 

Given the sophistication, collaboritve nature, and completness of Running Reaity, it's probably worth 
1. studying their approach, thinking about what we like or where we differ
2. maybe even having a call with Garth Henning (the creator) and seeing if there's a way we can directly build on his work



#### Historical maps sorta similar to Time Map
- [OmniAtlas](https://omniatlas.com/) made a series of static images (with overlay annotations) for an incredible swath of history and geography, I think it may be the most comprehensive set of static map images on the web. They don't have a single map, but generate a static map image per-continent, but seem to have (often?) a year-level of granularity.
...
- [worldhist.org](https://worldhist.org/) is highly detailed and extensive, going all the way back to prominent Nile river cities in 4000 BC. (despite their high quality, their data source links to an ancient looking Russian website of raw HTML tables of states...)
- [World History Encylopedia](https://www.worldhistory.org/map/) has videos, searchable data, and also maps, which they note "The map is only complete in the Mediterranean until around 270 BCE."
- [Ostellus Atlas](https://atlas.ostellus.com/?date=1800-01-01&rf=1775-05-12&rt=1824-08-23&zoom=4&lat=45.27488643704894&lon=18.017578125000004&id=17829307) has just from 1800 to present, but sports a cool draggable timeline with relevant historical events (eg wars) on it.
- [Open Historical Map](https://www.openhistoricalmap.org/#map=3/29.92/-68.38&layers=O&date=98-10-07&daterange=48-09-04,100-12-31) (part of OpenStreetMaps): "an ambitious, community-led project to map changes to natural and human geography throughout the world… throughout the ages." Data: "The underlying data is available for reuse as a download or through various free APIs compatible with open source software." and "As an open data project, OpenHistoricalMap encourages you to reuse the data in your own research and creative projects." Unclear how complete the data is, but this is the one project that really wants people to use their data *and* provides [a way](https://wiki.openstreetmap.org/wiki/OpenHistoricalMap/Reuse) to download all of it at once: "the entire contents of the OpenHistoricalMap database are available in planet.osm format on Amazon s3"
- [Euratlas](https://euratlas.com/) is Europe-specific
